...
SOURCE_SPREADSHEET = "DVS Water data Original.xlsx"
DESTINATION_SPREADSHEET = "DVS Water data Updated.xlsx"
...
def add_normalised_column(
    ...
    df[new_column_name] = df[target_column] - df[f"{target_column}_at_change"]
...
def convert_moisture_column(df: pd.DataFrame):
    df["Moisture Content g/g"] = df["Moisture content %"] / (
        100 - df["Moisture content %"]
    )


def add_pressure_change_columns(df: pd.DataFrame, pressure_solvent_column: str):
    """Add the columns based on pressure change-related to the DataFrame"""
    add_change_column(df, pressure_solvent_column, "Pressure Change")
    add_normalised_column(
        df, "Pressure Change", "Time [minutes]", "Normalised time (Individual RH Steps)"
    )
    convert_moisture_column(df)
    add_normalised_column(
        df,
        "Pressure Change",
...
def add_direction_change_columns(df: pd.DataFrame):
    """Add the columns based on direction change to the DataFrame"""
    add_change_column(df, "RH Direction", "Direction Change")
    add_normalised_column(
        df,
        "Direction Change",
        "Time [minutes]",
...
def calculate_and_add_columns(df: pd.DataFrame, pressure_solvent_column: str):
    """Calculate and add all necessary columns to the DataFrame"""
    add_pressure_change_columns(df, pressure_solvent_column)
    add_direction_change_columns(df)
...
def process_spreadsheet(src_spreadsheet: str, dst_spreadsheet: str):
    ...
    for sheet in src_workbook:
        ...
        calculate_and_add_columns(df, pressure_solvent_column)
